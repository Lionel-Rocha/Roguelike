...
class Enemy:
    def __init__(self):
        ghost_type = random.choice(['enemy1', 'enemy2', 'enemy3'])
...
def update():
    ...
    for projectile in projectiles[:]:
        ...
        for enemy in enemies[:]:
            if projectile.actor.colliderect(enemy.actor):
...
def draw():
    ...
    if menu:
        ...
        if game_over:
            screen.draw.text("Derrota!", center=(WIDTH // 2, HEIGHT // 2 - 40), fontsize=64, color=(68,204,255))
            screen.draw.text(f"Pontos obtidos: {score}", center=(WIDTH // 2, HEIGHT // 2 + 10), fontsize=36, color="white")
            screen.draw.text(f"Tempo final: {int(survival_time)}s", center=(WIDTH // 2, HEIGHT // 2 + 50), fontsize=36, color="white")
